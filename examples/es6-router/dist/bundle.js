...
/******/ 			script.src = __webpack_require__.p + "" + chunkId + ".bundle.js?v=2017216511";
	...
	JSpring.router('#container', {
	    ...
	    'mainPage': {
	        ...
	        controllerFn: function controllerFn(resolve) {
	            __webpack_require__.e/* require */(1, function(__webpack_require__) { var __WEBPACK_AMD_REQUIRE_ARRAY__ = [__webpack_require__(4)(BASE + 'mainPage/mainPage.js'), __webpack_require__(14)(BASE + 'mainPage/mainPage.css')]; (resolve.apply(null, __WEBPACK_AMD_REQUIRE_ARRAY__));}.bind(this));
	        },
	        title: '签证首页',
	        cach: true
	    },
	    ...
	    'visaList/:cityId': {
	        ...
	        controllerFn: function controllerFn(resolve) {
	            __webpack_require__.e/* require */(2, function(__webpack_require__) { var __WEBPACK_AMD_REQUIRE_ARRAY__ = [__webpack_require__(21)(BASE + 'visaList/visaList.js'), __webpack_require__(25)(BASE + 'visaList/visaList.css')]; (resolve.apply(null, __WEBPACK_AMD_REQUIRE_ARRAY__));}.bind(this));
	        },
	        title: '签证列表',
	        cach: false
	    },
	    ...
	    'visaDetail/:goodsId': {
	        ...
	        controllerFn: function controllerFn(resolve) {
	            __webpack_require__.e/* require */(3, function(__webpack_require__) { var __WEBPACK_AMD_REQUIRE_ARRAY__ = [__webpack_require__(28)(BASE + 'visaDetail/visaDetail.js'), __webpack_require__(32)(BASE + 'visaDetail/visaDetail.css')]; (resolve.apply(null, __WEBPACK_AMD_REQUIRE_ARRAY__));}.bind(this));
	        },
	        title: '签证详情',
	        cach: false
	    },
...
/***/ function(module, exports, __webpack_require__) {
	...
	;(function (global, factory) {
		( false ? 'undefined' : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? module.exports = factory() :  true ? !(__WEBPACK_AMD_DEFINE_FACTORY__ = (factory), __WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ? (__WEBPACK_AMD_DEFINE_FACTORY__.call(exports, __webpack_require__, exports, module)) : __WEBPACK_AMD_DEFINE_FACTORY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__)) : global.JSpring = factory(global);
	  ...
	  **/
		var WARN = {
			format: function format(attr) {
				return 'not match the format of {1}'.replace(/\{\d\}/g, attr);
			},
	  ...
	  **/
		var _ = {
			...
			getChildNodes: function getChildNodes(el) {
				return this.isElement(el) && el.childNodes;
			},
			...
			createLoadingImg: function createLoadingImg() {
				var str = '<div class="dataLoading">' + '<i></i><i></i>' + '<span>加载中...</span>' + '</div>' + '<div class="loadingMask"></div>',
				    div = $createEl('div');
			...
			proxyAjax: function proxyAjax(method, url, data, opts) {
				return _.ajax(url || '', this.lower(method || 'GET'), _.isObject(data) ? data : {}, opts || {});
			},
	   ...
	   **/
			...
			makeHashCode: function makeHashCode(prefix) {
				prefix = prefix || UNIQ;
				return String(Math.random() + Math.random()).replace(REGEXP.rhashcodeRE, prefix);
			},
			...
			replaceEscapeWord: function replaceEscapeWord(text) {
				return text.replace(/&nbsp;/g, '\xA0').replace(/&quot;/g, '"').replace(/&amp;/g, '&').replace(/&gt;/g, '>').replace(/&lt;/g, '<');
			},
			...
			arrEqual: function arrEqual(arr1, arr2) {
				return arr1.length == arr2.length && arr1.toString() === arr2.toString();
			},
	   ...
	   */
		...
		function optimizeCb(func, context) {
			...
			return func.apply(context, _.isArray(collection) ? collection : $slice.call(args, 2));
		};
		...
		function getNonMatchReg(value) {
			return new RegExp('\\s*(?:' + value.replace(REGEXP.spaceRE, '|') + ')\\b', 'g');
		};
		...
		function childrenDiff(prevChildren, newChildren, prevVNode, inst, childOpts) {
			...
			while (i + childOpts.nStep < newLen + newEnd || i + childOpts.pStep < prevLen + prevEnd) {
				inst.diff(newChildren[i + childOpts.nStep] || emptyDataObj, prevChildren[i + childOpts.pStep] || emptyDataObj, prevVNode, childOpts);
				i++;
			}
		};
			...
			if (sStatic) {
				...
				for (var key in sStatic) {
					...
					if (key != 'class' && key != 'style') {
						...
						tStatic[key] += (tStatic[key] ? key == 'class' ? ' ' : ';' : '') + sStatic[key];
					}
				}
			}
		...
		function createVObj(tagName, html, inst) {
			...
			while ((attrMatch = REGEXP.attrsRE.exec(html)) != null) {
				attrKey = attrMatch[1];
				attrValue = !_.isVoid0(attrMatch[2]) ? attrMatch[2] : !_.isVoid0(attrMatch[3]) ? attrMatch[3] : attrMatch[4];
		...
		function isBothObjOrArr(v1, v2) {
			return _.isObject(v1) && _.isObject(v2) || _.isArray(v1) && _.isArray(v2);
		};
		...
		function getChildResult(children, inst) {
			return children.length ? '[' + getMapResult(children, genVNodeExpr, inst) + ']' : '[]';
		};

		function genVNodeExpr(vObj, index, inst) {
			var data = getVObjData(vObj),
			    children = vObj.children,
			    text;
			...
			if (_.isElement(vObj)) {
				...
				return '__j._n(\"' + vObj.tagName + '\", ' + $stringify(data) + ', ' + getChildResult(children, inst) + ')';
			} else {
		...
		function genUniqVNodeExpr(vObj, inst) {
			...
			return '__j._n(\"' + vObj.tagName + '\", ' + res + ', ' + getChildResult(children, inst) + ')';
		};
	 ...
	 **/
		...
		Query.prototype = {
			...
			css: function css(cssHtml, value) {
				...
				if ((typeof cssHtml === 'undefined' ? 'undefined' : _typeof(cssHtml)) == 'object') {
	  ...
	  **/
		...
		VNode.prototype = {
			...
			update: function update(attr) {
				return optimizeCb(DIRECT[attr], this.instance, this.el, this.data.uniq[attr], this);
			},
	  ...
	  */
		...
		function genOn(vObj, value) {
			...
			return formatFlag ? '[' + tmpArr.join(', ') + ']' : (WARN.format('on'), STRING);
		};
		...
		function genFor(vObj, attrStr, inst) {
			...
			if (match = vObj.isFor.match(REGEXP.forExpRE)) {
				index = match[3];
				parent = match[2];
				str = '__j._mp(' + parent + ', function(' + match[1] + ', ' + (index || '$index') + ') {';

				if (!vObj.isComponent) {
					str += 'return __j._n(\"' + vObj.tagName + '\", ' + attrStr + ', ' + getChildResult(children, inst) + ');';
				} else {
					str += 'return ' + genComponent(vObj, attrStr, inst, parent, index || '$index');
				}
				str += '})';
				return str;
			}
	  ...
	  */
		...
		function vModel(el, value, vNode) {
			el = getIfElem(el, vNode);
			el.value = value[1];
			var uniq = vNode.data.uniq,
			    expression;

			if (!vNode.updateFn) {
				expression = !uniq.parent ? value[0] : REGEXP.replace(value[0], REGEXP.objParentRE, uniq.parent + (_.isVoid0(uniq.index) ? '' : '[' + uniq.index + ']'));
		...
		function _JSpring(propArr, opts) {
			...
			_this.data = (_.isFunction(_this.service) ? optimizeCb(_this.service, _this, [$, JSpring.module]) : _this.service) || {};
		...
		_JSpring.prototype = {
			...
			nextTick: function nextTick() {
				return callHook(this.cach.tickQ, this), this;
			},
			...
			genVNode: function genVNode() {
				return this.vNode = this.renderFn.call(this, this), this;
			},

			analyzeTplStr: function analyzeTplStr(vObj) {
				vObj = vObj || this.vObj;
				return 'with(this.$scope){ return ' + (vObj ? genVNodeExpr(vObj, 0, this) : "__j._n('div')") + ';}';
			},
				...
				if (initCompare.isBothText || initCompare.isBothCmt) {
					...
					if (_.arrEqual(nUKeys, oUKeys)) {
						nUKeys.length && _.each(nUKeys, function uKeysEach(ukey) {
							...
							if (isSimpleKey ? prevValue !== newValue : !_.proxyEqual(prevValue, newValue)) {
			...
			bootstrap: function bootstrap() {
				...
				_this.clearNoUseAttr();
				return optimizeCb(_this.controller, _this, _this.$scope, $, JSpring.module, _this); //, console.timeEnd('JSpring');
			}
		};
	   ...
	   **/
		...
		JSpring.router = function router(container, routes, cm) {
			...
			var isSwiperBack = JSpring.swiperBack ? function (x1, x2, y1, y2) {
				return Math.abs(x1 - x2) >= Math.abs(y1 - y2) && x2 - x1 > 100;
			} : function () {
				return false;
			};
			...
			if (container.exist) {
				...
				container.on('touchstart', function (e) {
					...
					if (Math.abs(pageX2 - pageX) > boundary || Math.abs(pageY2 - pageY) > boundary) {
						...
						if (JSpring.swiperBack && _.isApp && !hasSwiperBack && e.timeStamp - startTime > 300 && isSwiperBack(pageX, pageX2, pageY, pageY2)) {
			...
			function setTitle(title, hash) {
				...
				return JSpring.titleTag.html(_.isFunction(title) ? title(hash) : title || '');
			};
	   ...
	   **/
		...
		if (base.exist) {
			$location.base = /\.\//.test(base = base.attr('href')) ? base.replace(/\.([^\.]+)/, '$1') : base[base.length - 1] == '/' ? base : base + '/';
		}
