	  ...
	  **/
	   ...
	   */
		...
		function getChildResult(children) {
			return children.length ? '[' + getMapResult(children, genVNodeExpr) + ']' : '[]';
		};
		...
		function genVNodeExpr(vObj) {
			...
			if (_.isElement(vObj)) {
				...
				return '__j._n(\"' + vObj.tagName + '\", ' + $stringify(data) + ', ' + getChildResult(children) + ')';
			} else {
		...
		function genUniqVNodeExpr(vObj) {
			...
			return '__j._n(\"' + vObj.tagName + '\", ' + res + ', ' + getChildResult(children) + ')';
		};
	  ...
	  **/
		...
		function genFor(vObj, attrStr) {
			...
			if (match = vObj.isFor.match(REGEXP.forExpRE)) {
				return '__j._mp(' + match[2] + ', function(' + match[1] + ', ' + (match[3] || '$index') + ') {' + 'return __j._n(\"' + vObj.tagName + '\", ' + attrStr + ', ' + getChildResult(children) + ');' + '})';
			}
		...
		_JSpring.prototype = {
			...
			analyzeHtml: function analyzeHtml() {
				...
				while (match = REGEXP.startEndAngleRE.exec(_this.template)) {
					...
					if (parentTag) {
						...
						parentTag = isEndTag ? parentTag.parentVObj : !isNoEndTag ? vObj : parentTag;
					} else {
			...
			analyzeTplStr: function analyzeTplStr() {
				return 'with(this.$scope){ return ' + (this.vObj ? genVNodeExpr(this.vObj) : "__j._n('div')") + ';}';
			},
